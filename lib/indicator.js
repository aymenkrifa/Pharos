...
export const PharosIndicator = GObject.registerClass(
    class PharosIndicator extends PanelMenu.Button {
        _init(settings, uuid, path) {
            // 0.5 centers the menu under the button. The default 0.0 puts the
            // menu's left edge at the button's CENTER, which is invisible on a
            // one-icon indicator but throws the menu half a tag-width to the
            // right of ours — the tag is wide, and grows with every metric.
            super._init(0.5, 'Pharos');
        ...
        _recolorMeters() {
            for (const limit of this._lastLimits ?? []) {
                ...
                if (!meter)
                    continue;
                meter.setUsage(limit.pct, this._barColor(limit.pct));
            }
        }
        ...
        async _refresh(isRetry = false) {
            ...
            try {
                const data = await this._client.fetchUsage(
                    ...
                    { autoRenew: this._settings.get_boolean('auto-renew-signin') });
