...
//   isActive true when the API flags this limit as the one currently binding
...
export function parseLimits(data) {
    ...
    if (data?.five_hour) {
        limits.push({
            ...
            resetsAt: data.five_hour.resets_at ?? null, isActive: false,
        });
    }
    if (data?.seven_day) {
        limits.push({
            ...
            resetsAt: data.seven_day.resets_at ?? null, isActive: false,
        });
    }
    return limits;
}
