...
export class UsageClient {
    ...
    // With autoRenew false (the read-only mode setting) both refresh paths
    // are off: no request to the token endpoint, no write to disk — the
    // stored token is used as-is until Claude Code itself renews it.
    // Throws UsageError, or the raw Gio CANCELLED error when aborted.
    async fetchUsage(account, cancellable, { autoRenew = true } = {}) {
        let creds;
        try {
        ...
        if (autoRenew && creds.oauth.refreshToken && expiresAt &&
            ...
            if (e instanceof UsageError && e.kind === 'auth' && creds.oauth.refreshToken) {
                if (!autoRenew) {
                    throw new UsageError('auth',
                        'Sign-in expired — auto-renew is off, so open Claude Code to renew it.');
                }
                const token = await this._refreshToken(creds, cancellable);
                return this._requestUsage(token, cancellable);
            }
    ...
    async _refreshToken(creds, cancellable) {
        ...
        // A rate-limited refresh is transient, not a dead credential — hand it
        // to the same soft path the usage endpoint uses (keep the shown data,
        // retry quietly) rather than telling the user to re-auth.
        if (msg.status_code === 429)
            throw new UsageError('rate-limit', 'Rate limited');

        if (msg.status_code !== 200) {
            // Surface the server's OAuth error (e.g. {"error":"invalid_grant"})
            // — a failed refresh returns no tokens, so the body is safe to
            // show and tells us WHY (dead token vs. malformed request).
            const detail = new TextDecoder().decode(bytes.get_data()).trim().slice(0, 300);
            logError(new Error(`pharos token refresh ${msg.status_code}: ${detail}`),
                'pharos: token refresh');
            // invalid_grant specifically means the refresh token is dead and
            // only a fresh sign-in fixes it; other 4xx are worth reporting raw.
            const dead = detail.includes('invalid_grant');
            throw new UsageError('token-refresh', dead
                ? 'Sign-in expired — open Claude Code and sign in again.'
                : `Token refresh HTTP ${msg.status_code}: ${detail || 'Run Claude Code to re-auth.'}`);
        }
